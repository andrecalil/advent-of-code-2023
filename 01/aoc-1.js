...
for (const line of lines) {
  ...
  for (const replace of digitsAsWords) {
    preparedLine = preparedLine.replaceAll(
      replace.word,
      `${replace.word[0]}${replace.digit}${replace.word.substring(1)}`
    );
  }

  for (const letter of preparedLine) {
    if (!isNaN(letter)) currentWord += "" + letter;
  }

  const lineArray = currentWord.split("");
  const length = lineArray.length;

  digit = `${lineArray[0]}${lineArray[length - 1]}`;
